...
class SubscribtionsItem extends StatelessWidget {
        ...
        startActionPane: ActionPane(motion: DrawerMotion(),
          children: [
            SlidableAction(onPressed: (BuildContext cotext)  async {
              ...
              print("${sub.planId}");
              try {
                plan = await ApiManager.fetchPlanData("/subscription/getPlan/${sub.planId}");
              } catch (e) {
                print('Error: $e');
              }
              if(plan!=null)
              Navigator.push(context, MaterialPageRoute(builder: (context) => PlanDetails(plan: plan!)));
              else
                print("${plan?.id}");
            },
              spacing: 15,
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      SizedBox(width: 5,),
                      Text("Plan ID : ${sub.planId}",style: TextStyle(fontSize: 17,
                      //  fontWeight: FontWeight.w500,
                        color:Colors.white,)),
                      SizedBox(height: 5,),
